    ...
    public function getPrecisePricing(array $criteria): array
    {
        $payload = [
            ...
            'pricing' => [
                ...
                'children'    => $criteria['children'] ?? 0,
                'infants'     => $criteria['infants']  ?? 0,
                // USE the provided solutionId/solutionKey when present:
                'solutionId'  => $criteria['solutionId']  ?? 'direct pricing',
                'solutionKey' => $criteria['solutionKey'] ?? '',
                // Pass requested cabin if provided (PK accepts blank if unknown)
                'cabin'       => strtoupper($criteria['cabin'] ?? ($criteria['cabinType'] ?? '')),
                'tag'         => $criteria['tag'] ?? null,
                // Only include journeys when we have to (i.e., no solutionId/solutionKey)
                'journeys'    => [],
            ],
        ];
